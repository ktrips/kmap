...
第17章のケーススタディでは、最初のリリースにたどり着くまでの拡張の歴史を振り返りました。本章では、それに続く「2つめ以降の週末」で実際に追加した機能を、実装の勘所とともに紹介します。派手な新機能よりも、こうした地道な拡張・磨き込みの積み重ねこそが、リリース後のアプリを成長させる原動力になります。

### 22-1. 連携カメラ・連携プリンター：外部のIoT機器と連携する

M5StackのようなIoT機器を、自宅Wi-Fi上の「もう1つのカメラ」「もう1つのプリンター」としてアプリに組み込む機能を追加しました。設計のポイントは次の3つです。

**1. 設定はホスト名だけ、パスはアプリ側で組み立てる**

「設定」画面ではユーザーにホスト名／IP（例：`m5web.local`）だけを入力してもらい、実際のAPIパス（`/api/print/photo`など）はアプリ側の定数として管理します。ユーザーが古い設定値（別のAPI仕様を試していた時の値）を消し忘れていても、`URLComponents`で一度分解してスキーム・ホスト・ポートだけを取り出し直すことで、意図しないURLが組み上がるのを防いでいます。
...
**2. 転送方式を複数用意し、相手のAPI仕様に合わせて選べるようにする**

連携プリンターのAPIには、「画像データを直接受け取るタイプ」（`multipart/form-data`でのPOST）と「画像のURLだけを受け取り、自分で取得しにいくタイプ」（GETにクエリパラメータでURLを渡す）の2通りが実務では存在します。後者のためには、画像を一時的にクラウドストレージへアップロードしてURLを発行する必要があります。両対応にしておくことで、相手の実装に合わせて設定を切り替えるだけで済むようにしました。
...
IoT機器との通信でありがちな落とし穴として、`URLError.networkConnectionLost`（「network connection was lost」）が挙げられます。多くの場合、次の2つの組み合わせで解決します。

- `Connection: close`ヘッダーを明示し、iOS側にコネクションの使い回し（keep-alive）をさせない
- ボディ付きPOSTで自動付与される`Expect: 100-continue`ヘッダーを、`Expect`ヘッダーを自分で（空文字で）明示することで抑止する（簡易的なESP32/Arduino系のWebサーバーはこのヘッダーに対応できず、接続を切ってしまうことが多い）

さらに、HTTPステータスコードを確認せずに「リクエストが例外を投げなければ成功」と判定してしまうと、実際にはプリンター側が404や500を返していても「送信しました」と表示されてしまいます。レスポンスの`HTTPURLResponse.statusCode`を必ず確認し、2xx以外は失敗として扱うようにしましょう。
...
エラーメッセージには、可能な限り「原因」を含めることをお勧めします。「送信に失敗しました」だけでは、ユーザー（そして未来の自分）はどこから調査すればいいか分かりません。通信エラーの`localizedDescription`やHTTPステータスコードをそのまま画面に出すだけで、トラブルシューティングの速度が大きく変わります。

### 22-2. 投稿写真の管理：フリック・削除・非公開

写真は「撮って終わり」ではなく、後から見返し、時には削除したり公開範囲を調整したりするものです。既存の1枚だけのプレビュー画面を、次のように拡張しました。

**同じグループ内の写真をフリックで移動できるようにする**

SwiftDataの`@Query`を使い、同じ`walkRouteID`を持つ写真を動的にフィルタして取得し、`TabView`の`.page`スタイルでページングします。ポイントは、**表示対象の一覧を親から渡された固定配列ではなく、`@Query`で直接監視する**ことです。これにより、シートの中で1枚削除しても、一覧が自動的に更新されて隣の写真にシームレスに移動できます。
...
「みんなの時空旅」に公開中の記録であっても、特定の1枚の写真だけは公開したくない、という要望は自然に発生します。これを実現するために、写真1枚ごとに`isHiddenFromSharing`という真偽値を持たせ、公開データを作り直す処理（`setPubliclyShared`）の中でこのフラグが立った写真を除外するだけにしました。

```swift
for post in photoPosts where post.photo != nil && !post.isHiddenFromSharing {
    // 公開データ（sharedTrips配下）に含める
}
```

削除・非公開化のどちらも、それが属する時空旅が既に公開済みであれば、公開データを都度作り直す（再同期する）処理を呼び出す必要があります。この「編集のたびに公開データを再同期する」というパターンは、本書を通じて何度も登場しています。編集操作を追加するたびに「これは公開データにも影響するか？」を自問する習慣をつけておくと、公開・非公開の整合性が崩れる不具合を防げます。
...
第4章で紹介した「歩いた場所だけくっきり見せる」演出（宝探し効果）は、実装当初、ぼかしの強さ（ガウスぼかし半径14）と、まだ通っていない場所の不透明度の下限（0.6）の組み合わせが強すぎて、「古地図がぼんやりとしか見えない」というフィードバックにつながりました。対策として、ぼかし半径を5まで弱め、不透明度の下限を0.88まで引き上げました。ゲーム性（宝探し感）と実用性（古地図として見やすいこと）のバランスは、こうした数値のチューニングによって初めて着地点が見つかります。最初の実装値を「仮の値」として割り切り、リリース後に実際のフィードバックで追い込んでいく姿勢が重要です。

**ズーム・パン後にオーバーレイが消える不具合**

Google Maps SDKには、大きくズーム・パンした直後に`GMSGroundOverlay`のテクスチャがGPU側で失われたまま再描画されない、という既知の癖があります。対策は、カメラの移動が収まった（`idle`）タイミングで、オーバーレイを重い再デコードなしに一旦外して貼り直すことです。
...
このような「SDKの内部実装に起因する消失・再描画不良」は、実機で長時間・様々な操作を試してみないと見つからないことが多く、机上のテストだけでは気づけません。リリース後、実際のユーザーの操作パターン（大きくズームする、地図を素早くドラッグするなど）から不具合が報告されたら、「操作の直後に強制的に再描画し直す」という対症療法的な対策から試してみる価値があります。

### 22-4. Web版をマーケティングの入り口にする

第II部で扱ったマーケティングの考え方を、実際にWeb公開ページ（`komap.ktrips.net`）に落とし込んだ実例を紹介します。

**Web版は「見るだけ」の体験、iOSアプリが「本体験」——役割を明確に分ける**

未サインインの訪問者向けのWeb公開ページは、「みんなの時空旅」を眺められる入り口として設計されていますが、それだけで終わらせず、**iOSアプリへの移行を強く後押しする導線**を組み込みました。

- ヘッダーの「使い方」ボタンから開くモーダルでは、Web版でできることはあえて1〜2文に圧縮し、iOSアプリでできること（実際に歩いて古地図を発見する、AIが物語を語る、御朱印を集める、外部機器と連携する）をカード形式で強調しています。「このWebページで見られるのは、iOSアプリで生まれた記録の“一部”です」という一文で、Web版がゴールではなく入り口であることを明示し、末尾に「Googleでサインインして、iOSアプリを始める」ボタン（サインインすると、既存の仕組みでTestFlightの招待メールが自動送信される）を置いています
- 当初は公開ページ下部にも独立した「iOSアプリをダウンロード」ボタンを置いていましたが、同じ画面に似た誘導ボタンが並ぶと逆にどれを押せばいいか伝わりにくくなったため、後日取り除きました。誘導は「使い方」モーダル1箇所に集約し、そこでしっかり訴求する方針に落ち着いています。マーケティング施策は追加するほど良いとは限らず、効果が重なる導線は思い切って削る判断も必要です

**書いている本そのものを、マーケティングコンテンツにする**

本書はKindleでの出版を目指して執筆されていますが、完成を待たずに、執筆中の原稿の冒頭部分を「Komapの作り方 Kindle（一部無料）」というボタンからWeb公開ページ上でそのまま読めるようにしました。原稿全文をクライアントに配信するのではなく、あらかじめ冒頭部分だけを切り出した専用ファイルを配信することで、「続きが気になる」という興味を安全に喚起しつつ、原稿全体の流出を防いでいます。
...
この施策は、第14〜16章で紹介した「開発の過程そのものをコンテンツにする」というマーケティング手法の延長線上にあります。個人開発者にとって、開発記録・執筆中の原稿・裏側の試行錯誤は、追加のコストをほとんどかけずに用意できる、最も飾らない形のマーケティングコンテンツです。

### 22-5. パフォーマンスの継続的な改善——「動いているから十分」で終わらせない

機能追加が一段落した後も、実際の利用シーンを想像しながらパフォーマンスを見直す作業は続きます。ここでは、リリース後に見つかった2つの具体的な事例を紹介します。

**Web版：無関係な再描画が、地図をまるごと作り直させていた**

Reactでは、propsに渡す配列やオブジェクトを`useMemo`で固定しておかないと、コンポーネントが再描画されるたびに「新しい配列」が作られ、それを受け取った子コンポーネントの`useEffect`が「依存関係が変わった」と誤検知することがあります。時空旅の詳細画面では、地図に重ねる史跡チェックポイントの一覧を次のように毎回その場で計算していました。
...
`useMemo`で固定するだけの、1行の修正です。修正の効果は、ブラウザの開発者ツールで`GroundOverlay`のコンストラクタ呼び出し回数を実際に数えて検証しました——「直しました」で終わらせず、狙った再描画が本当に減ったかを数値で確認する一手間が、こうした性能問題では特に重要です。

**iOS版：フル解像度画像の縮小処理が、メインスレッドをブロックしていた**

古地図を切り替えるたびに、3000pxを超えることもあるフル解像度の画像を1600pxまで縮小する処理を、これまでメインスレッドで同期的に実行していました。この処理自体は「見た目には」問題なく動きますが、実行中はUIの描画が一瞬止まるため、「地図タブを開いた瞬間」や「歩行記録開始時に古地図が自動表示される瞬間」に、体感できる「もたつき」として現れていました。

対策は次の2段構えです。

1. 縮小結果を古地図IDごとにキャッシュし、2回目以降の表示では即座に使い回す
2. 未キャッシュの初回は、まず画像なし（枠だけ）のオーバーレイを即座に表示しておき、縮小処理はバックグラウンドスレッドで行ってから、完了時に画像だけを差し替える

```swift
if let cached = Self.singleOverlayImageCache[overlayID] {
    // キャッシュ済みなら即座に使う
} else {
    // まず枠だけ表示 → バックグラウンドで縮小 → 完了後にiconを差し替え
...
**iOS版：GPS更新のたびの処理が、歩行時間に比例して重くなっていた**

歩行記録中は数秒おきにGPSの新しい座標が届き、そのたびに「軌跡を滑らかに描き直す」「古地図の“めくれ”演出を合成し直す」処理が走ります。当初の実装は、どちらも新しい座標が1件増えるたびに**蓄積した軌跡全体**を対象に計算をやり直していました。

```swift
// 修正前：新しい点が1つ増えるだけで、蓄積済み全体を毎回スムージングし直す
let path = GMSMutablePath()
Self.smoothedTrailCoordinates(live).forEach { path.add($0) }  // liveは記録開始からの全座標
```

歩き始めた直後はほとんど気づきませんが、30分・1時間と歩き続けるうちに`live`（記録済みの全座標）の要素数がどんどん増え、GPS更新1回あたりの計算量も比例して増え続けます。「歩き始めは快適だったのに、長く歩くとだんだんカクつく」という、時間が経たないと表面化しない典型的な性能劣化です。

対策は、**新しく増えた区間だけを対象に、既存の結果へ差分で追記する**ことです。
...
for index in (previousCount - 1)..<(live.count - 1) {
    let p0 = live[index]
    let p1 = live[index + 1]
    path.add(/* p0とp1のスムージング済み中間点 */)
}
path.add(live[live.count - 1])
```

古地図の「めくれ」演出（通った場所だけくっきり見せる画像合成）も同様に、前回合成済みの画像を土台にして新区間だけ追加で描き足す方式に変更しました。どちらも、GPS更新1回あたりの計算量が「これまで歩いた距離」ではなく「前回からの移動量」だけに依存するようになり、歩行時間が伸びても体感速度が変わらなくなります。

もう1つ、地図をズーム・パンした直後に古地図のテクスチャが消えて見えるGPU側の不具合への対策（オーバーレイをいったん外して貼り直す）も、当初はカメラが止まるたび毎回実行していました。これはチェックポイントのマーカーを選択して開いた名前表示まで巻き添えで閉じてしまう副作用があり、「チェックポイントをタップしても、名前がすぐ消える」という別のバグとして報告されました。原因は、貼り直し処理がマーカーの`.map`をいったん`nil`にしてから戻す実装になっており、これがGoogle Maps SDK上でマーカーの選択状態を解除してしまうためでした。ズームが一定以上変化した時だけ貼り直すよう条件を絞ることで解決しています。
...
以前はサインイン後の「時空旅」タブを自分の記録だけに絞っていましたが（第20章のティール型戦略で触れた「独占ニッチ」を強く意識しすぎた設計でした）、実際に使ってみると「サインインすると、それまで見えていたみんなの時空旅が見えなくなる」という体験の分断が気になり、自分の記録と他ユーザーの公開記録を再び一緒に一覧できるように戻しました。公開されている記録には🌐アイコンを付け、「これは誰でも見られる記録だ」と分かるようにしています。機能の追加・削除は一直線には進まず、実際に使ってみて「行き過ぎた」と感じたら素直に戻す判断も、個人開発の大事な一部です。

### 22-6. 旅日記機能：既存のAI生成資産を再利用する設計

歩いた記録を「その場で振り返る」体験をさらに一歩進め、時間旅の詳細画面に「旅日記を作成する」ボタンを追加しました。旅の名前・使った古地図・巡った御朱印スポット・投稿写真・ユーザー自身の感想を材料に、AIがエッセイ調の旅日記をMarkdown形式で書き上げ、生成後は同じ画面から「旅日記を読む」ボタンでいつでも読み返せます。

**新しいAI呼び出しを増やす前に、既にある生成物を探す**

このアプリには、史跡チェックポイントを開くと「その場所の昔の物語」をAIが生成し、`CheckpointStory`として端末に保存しておく仕組みが既にありました（同じ場所を何度開いても再度AIに問い合わせない、という利用料対策の設計です）。旅日記生成でも、巡った御朱印スポットそれぞれについて安易に新しいAI呼び出しを追加するのではなく、まず「その場所の詳細は既に生成済みか」を`CheckpointStory`から確認し、あればそれをほぼそのまま旅日記に取り込み、無い場合だけその場で生成してキャッシュに保存する設計にしました。
...
if let existing = fetchSavedStory(siteID: site.id, modelContext: modelContext) {
    ...
    let story = try await historyService.generateStory(for: site.coordinate, overlayMap: overlayMap, placeName: site.name)
    modelContext.insert(CheckpointStory(siteID: site.id, title: story.title, body: story.body))
}
```

この設計には2つの利点があります。1つは単純にAPI利用料と待ち時間の節約です。もう1つは、**アプリの別の画面（チェックポイント詳細シート）と旅日記とで、同じ場所について書かれる内容が一致する**という体験上の一貫性です。プロンプト側にも「取得済みの文章は要約・言い換えをしない」という指示を明示的に与え、AIが同じ場所について毎回違う言い回しを作り出してしまわないようにしています。既存の機能を作るときに用意した「一度生成したら再利用する」という設計判断は、後から生えてくる新機能にとっても資産になる、という一例です。

**AI呼び出しはどこで行うか——クライアント側かサーバー側か**

旅日記生成のAI呼び出しは、既存の物語生成（`AIHistoryService`）と同じく、iOSクライアントから直接OpenAI APIを叩く方式を踏襲しました。Cloud Functions経由でサーバー側からAI APIキーを使う設計（キーを端末に持たせず、利用量の管理もしやすい）も選択肢としてはありましたが、この機能のためだけに新しいCloud Functionsのデプロイ・シークレット管理を増やすコストと、既存の設定画面・Keychain保存の仕組みをそのまま使える手軽さを比べ、後者を選びました。機能ごとに「一貫性を取るか、その機能に最適な設計を取るか」を都度判断する必要があり、今回は既存パターンの一貫性を優先しています。

**文章だけでは物足りない——スクラップブック形式への作り直し**

最初のバージョンは、AIが書いたエッセイをそのまま1本のMarkdownとして表示するだけでした。実際に読んでみると、文章の中に「桜田門を過ぎ、和田倉門へ向かった」と書かれていても、対応する御朱印の写真や歩いたルートの地図がすぐ横に見えないと、記録としての実感が薄いことに気づきました。そこで旅日記の画面を、AIの文章に加えて、実際に歩いたルートの地図・巡った御朱印（写真＋その場所の説明）・投稿写真（写真＋説明）を並べた、スクラップブックのような構成に作り直しました。
...
ここでのポイントは、この「写真＋説明」の一覧を新しいデータ構造として別途持たせるのではなく、御朱印なら`CheckpointStory`、投稿写真なら`WalkPhotoPost.storyBody`という、**既にアプリの中に存在する情報をそのまま並べ直しただけ**という点です。旅日記という新しい画面のために新しいデータを追加で作り込む必要はなく、既存のデータを「1つの画面にまとめて見せる」ビューを1つ書けば済みました。

**生成した内容を、そのままWeb版にも流す**

旅日記の本文（Markdown）は`WalkRoute`に保存され、既存の同期の仕組み（`users/{uid}/walkRoutes`・公開時は`sharedTrips`）にフィールドを2〜3個足すだけで、Web公開ページ（`komap.ktrips.net`）側にもそのまま伝わるようにしました。Web側では新しくAIを呼び出したり、内容を作り直したりはせず、iOSで生成済みのMarkdown本文を、Kindle原稿プレビューで使っているのと同じ`marked`によるMarkdown→HTML変換にそのまま渡すだけです。

```tsx
const journalHtml = marked.parse(trip.journalMarkdown, { async: false }) as string;
```

写真＋説明のスクラップブック表示についても、Web版で同じ体験を再現したくなりますが、Web側は普段iOS端末上にしかない`CheckpointStory`（御朱印スポットの説明）を直接読むことができません。ここで新しいFirestoreコレクションを作って同期の仕組みを1つ増やすことも検討しましたが、既に「みんなの時空旅」を公開する際に御朱印・投稿写真の画像をコピーして`sharedTrips`に載せている処理（`setPubliclyShared`）があったため、そこに説明文（`detail`）を1フィールド足して一緒にコピーする方式にとどめました。公開のタイミングで一度だけ複製される非正規化したデータですが、「新しい同期の仕組みを1つ増やす」より「既にある同期処理にフィールドを1つ足す」方が、変更範囲もテストする経路も小さく済みます。

「1箇所で生成し、複数のチャネルにそのまま流用する」という発想は、22-4で触れたKindle原稿プレビューの配信方法とも共通しています。新しいコンテンツの置き場所を増やすたびに、そこでしか読めない専用の生成ロジックを作るのではなく、**単一の生成物を複数の画面・チャネルで使い回せるように設計しておくと、後から表示先を増やす作業がとても軽くなります**。

**リリース後も、細かいフィードバックを1つずつ形にしていく**

旅日記機能は、最初の実装で終わりではありませんでした。実際に使ってみて出てきた具体的な要望を、その都度小さな修正として積み重ねています。

- AIが書く本文が、各史跡の詳細を丸ごと引用する長いエッセイだったため、御朱印・写真のギャラリー側と内容が重複して読みにくくなっていました。プロンプトを書き直し、個々の詳細はギャラリー側だけに任せて、本文は旅全体の印象を200字程度でまとめる「旅のサマリー」に絞りました
- 「時空旅を開いた時の画面」と「旅日記画面」で、名前・公開状況・日付・距離・件数といった同じ種類の情報の並び順がバラバラだったため、両方のヘッダーを完全に同じ構成・同じ並び順に揃えました（その後、旅日記画面だけ名前横の古地図名を外し、代わりにサマリー見出し「（古地図名）の時空旅」へ意図的に移すという細部の作り分けも行っていますが、これは揃えた後だからこそできた「意図した差分」です）
- Web版は当初、地図・御朱印ギャラリー・投稿写真ギャラリーが「一般的な一覧セクション」と「旅日記セクション」の2箇所に重複して存在していました（旅日記の有無に関わらず表示される簡易版と、旅日記がある時だけ追加される詳細版）。地図を2つ読み込む無駄も含め、これを1本の流れに統合し、旅日記の有無に関わらず同じ構成で表示するようにしました
- 「みんなの時空旅」を未サインインで開いた訪問者に画像が表示されない不具合も見つかりました。原因はFirebase Storageのルールで、対応するFirestoreドキュメントは公開読み取り可にしていたのに、画像本体（Storage側）だけサインイン必須のままになっていました。同じ「公開」という状態を、Firestoreとstorageの2箇所のルールで別々に管理していたために、片方だけ直し忘れていたのです。関連するリソースの公開範囲は、片方を変えたらもう片方も揃っているか必ず確認する癖が要ります
- 個別の時空旅を`?trip=<id>`というURLで直接開けるようにもしました。ルーティングライブラリを新規に入れるほどの規模ではなかったため、`URLSearchParams`と`history.pushState`だけを使う小さなカスタムフック（`useSelectedTripId`）を1つ書き、選択状態とURLを双方向に同期させています。「共有できるURLが無い」ことは機能不足には見えにくいものですが、実際には「この時空旅、誰かに見せたい」という一番自然な欲求を塞いでいました
- 公開ページ右上の「使い方」ボタンは、個別の時空旅を開いている間はサインインの案内（ページ上部の案内文）が隠れてしまうため、その間だけ「ログイン」ボタンに差し替えました。ボタンの色もGoogleサインインボタンと揃え、「今何をすべきか」が一目で分かるようにしています
- 「今N人が時空旅中」という閲覧人数の表示は、当初ヘッダー中央に単独で置いていましたが、Komapのアプリアイコン・サービス名のすぐ右横（一覧表示時のみ）に移し、「アイコン→サービス名→閲覧人数」の順に揃えました。関連する情報同士を近づけるだけで、「今Komapを使っている人がこれだけいる」という文脈が伝わりやすくなります
- 個別の時空旅を選んで詳細を表示している間は、一覧側の「N件の旅」という件数表示を隠すようにしました。今見ている内容と無関係な数字をヘッダーに残しておくより、空いた分だけ右側の要素（サインインボタン等）を右へ寄せた方が、限られた横幅を有効に使えます
- 日時の表記が画面ごとにバラバラ（「2026年9月10日」「9/10 14:30」など）になっていたため、`YYYY/M/D HH:MI`という1つの形式に統一しました。地味な変更ですが、複数の画面を行き来するユーザーにとって表記の揺れは「作り込みの甘さ」として伝わりやすい部分です
- TestFlightへの招待送信がAppleのAPI仕様上の理由（`STATE_ERROR`）で失敗した時、以前は技術的なエラーコードをそのまま表示していましたが、ユーザーには「招待の準備に問題が発生しました」という平易な案内に変え、代わりに開発者（管理者）宛にその場で報告メールを送れる`mailto:`リンクを添えました。ユーザーに不要な技術詳細を見せず、それでも問題を報告する手段は残しておくバランスです

いずれも機能としては地味な調整ですが、「同じ情報は同じ場所・同じ順で見せる」「重複した実装を1つにまとめる」「関連する設定は揃っているか確認する」「画面の状態によって、今ユーザーに一番押してほしい導線を出し分ける」という、当たり前だけれど気を抜くと崩れる原則の積み重ねです。個人開発では大きな新機能よりも、こうした小さな揃え直しの数のほうが、実際の使い心地に効いてきます。

### 22-7. 本を実際に売り始める、旅の記録をアプリの外へ連れ出す

**「執筆中の」が取れる瞬間**

本書はしばらく「執筆中のKindle原稿」としてプレビューだけを公開していましたが、実際にAmazonで販売を開始したタイミングで、アプリ内・Web版のあらゆる箇所から「執筆中の」という文言を取り除き、代わりにAmazonへの購入リンクを追加しました。iOSアプリの「設定」→「このアプリについて」、Web版のKindle原稿プレビューモーダル（本文末尾・冒頭のリンクの両方）、公開ページのサイドバー最下部——導線を1箇所に絞るのではなく、既にユーザーがKindle原稿に興味を持って辿り着いた場所すべてに、購入までの最後の一手を用意しています。プレビューを読んで満足してもらうことがゴールではなく、そこから実際に「続きを読みたい」と思った人が迷わず買えることがゴールだからです。

**旅の記録を、アプリの外に連れ出す**

「みんなの時空旅」で公開している時空旅は、これまでWebページを直接開いてもらうことでしか見せられませんでした。旅の記録は本来、歩いた本人が「これ見て」とLINEやSNSで誰かに見せたくなるものです。そこで、時空旅の詳細画面に共有ボタンを追加し、地図（古地図画像に歩いたルートを重ねたもの）・名前・距離・時間・説明・御朱印・写真を1枚の画像にまとめて生成し、Web版の個別URLを短縮した上で紹介メッセージと一緒に、標準の共有シートから送れるようにしました。

ここでの技術選択は「実行中のGoogle Mapをスクリーンショットしない」ことです。ライブの`GMSMapView`をその場でキャプチャする方法も検討しましたが、タイルの非同期読み込みを待つ必要があり、共有ボタンを押した瞬間に確実な画像を返せる保証がありません。代わりに、既に端末上にある古地図画像（ネットワーク不要）とルートの座標配列だけを使い、SwiftUIの`Canvas`で緯度経度を単純な線形変換でピクセル座標に写して描画しています。回転（bearing）のある一部の古地図では厳密さを欠きますが、「シェア用の要約カードとして十分な近似」と「即座に・確実に生成できること」を天秤にかけ、後者を優先しました。

URLの短縮には、Firebase以外の第三者サービス（TinyURLの認証不要API）を初めて使いました。新しいサービスを1つ増やすたびに「もし落ちていたら」を考える必要がありますが、短縮に失敗した場合は元の長いURLをそのまま使うフォールバックを用意することで、シェア機能自体が丸ごと止まることは避けています。外部サービスに依存する機能を足す時は、その依存先が使えなかった場合にどう振る舞うかを、必ず実装の一部として考えておく必要があります。

**シェア画像とURL短縮を、両方とも作り直す**

リリース直後から2つの不満が見えてきました。1つは、シェア画像の地図が「簡易描画」だと分かってしまうことです。画面で見ていた古地図の傾き・重なりと、共有画像の地図の見た目が微妙に違うため、せっかく共有しても「なんだか安っぽい」印象を与えていました。もう1つは、TinyURLへの外部依存そのものです。認証不要のAPIは変更や停止の予告なしに挙動が変わることがあり、実際に短縮が不安定になる場面がありました。

そこで両方を作り直しました。地図画像は、Canvasでの再描画をやめ、画面に既に表示中の`GMSMapView`（現在地図＋古地図オーバーレイ＋歩いたルート＋御朱印マーカーがすべて重なった状態）を`drawHierarchy(in:afterScreenUpdates:)`でそのままスナップショットする方式に変更しました。GMSMapViewはMetal/OpenGLで描画されるため、一般的な`layer.render(in:)`ではキャプチャできない点が落とし穴でしたが、`drawHierarchy`はスクリーン更新後のビュー階層を丸ごと描画し直すため問題なく撮れます。「今まさに画面に見えているものを、そのまま画像にする」というシンプルな方針に倒したことで、簡易描画との見た目の差というそもそもの不満を解消できました（取得できなかった場合だけ、従来のCanvas簡易描画にフォールバックします）。あわせて、御朱印・投稿写真も件数上限を廃止し、時間旅の記録画面と同じく全件を並べるようにしています。

URL短縮は、外部サービスに頼らない方式に切り替えました。時空旅のIDはFirestore側ではUUID文字列（36文字）ですが、UUIDは実質16バイトの情報しか持っていません。この16バイトをBase64URLエンコードするだけで22文字の短縮IDになり、しかもサーバー側の対応表を持たずに双方向変換できます（`UUID+ShortID.swift`／Webの`tripShortId.ts`に同じロジックを実装し、iOS・Web間で相互運用可能にしています）。第三者サービスへの依存も新たなサーバー実装も増やさずに、TinyURLと同程度の短さを達成できました。以前発行した`?trip=<UUID>`形式のリンクも、Web側で両方のパラメータ名を読み取ることで引き続き開けます。

ただし、この切り替えで新しいバグを1つ生みました。短縮IDからUUIDを復元する処理が、16進数を小文字で組み立てていたのに対し、iOS側が書き込むFirestoreのドキュメント名（`UUID.uuidString`）は大文字だったため、両者が文字列として一致せず「共有リンクを開いても旅日記が見つからない」という不具合になりました。可逆変換の実装では、変換ロジックそのものの正しさだけでなく、**変換後の値を実際に使う側（今回はFirestoreのドキュメントID）のフォーマットと完全に一致しているか**まで確認する必要があります。

**同じバグが2度目に出た時の対応**

22-3で紹介したGoogle Maps SDKのGPUテクスチャ喪失バグ（古地図が透明になって消える）は、ズーム操作からの復帰（`idleAt`でのワークアラウンド）で一度対応していましたが、「歩行中に古地図が消えて、古地図メニューから選び直しても直らない」という報告が改めて入りました。原因は、選び直した古地図が既に選択中のものと同じ場合、内部の「古地図が変わったかどうか」の判定（IDの比較）が`false`になり、オーバーレイを作り直す処理そのものがスキップされていたことでした。カメラ移動のリクエスト（`CameraMoveRequest`）で既に使っていた「値そのものではなく、操作のたびに新しいIDを発行するリクエスト」というパターンを流用し、同じ古地図の選び直しでも新しいリクエストとして検知できるようにしています。1つのバグに複数の症状（ズーム時・選び直し時）があり得ることを前提に、直した後も「この修正でカバーできていない再現条件はないか」を疑う視点が必要でした。

**SwiftUIの型検査タイムアウトが、再び起きた**

`MapScreen`の`body`は、地図・パネル・複数のシート・複数の`onChange`を1つの巨大な修飾子チェーンとして書いています。22-5より前にも、この`body`が「The compiler is unable to type-check this expression in reasonable time」というコンパイルエラーを起こしたことがあり、その時は`onChange`群を`CacheSyncObserver`という別Viewに切り出して解決していました。今回は、`.alert(isPresented:)`にインラインで書いていた`Binding(get:set:)`が同じ症状を引き起こしました。SwiftUIの型推論は、深くネストした巨大な式を一度に解決しようとするため、式のどこか1箇所が複雑なだけで全体が指数関数的に時間がかかることがあります。対処は今回もシンプルで、複雑な部分（`Binding`の生成）を独立した計算プロパティに切り出すだけです。教訓は、「1つの`body`に機能を継ぎ足し続けると、いつか型検査の限界を超える」という前提を持ち、コンパイルエラーが「あなたのロジックの複雑さの警告」だと捉えることです——エラーメッセージ自体は原因の場所を正確には教えてくれないため、直近で追加した複雑な式（クロージャ、三項演算子の入れ子など）から疑って1つずつ切り出していく、地道な切り分けが必要になります。

### 22-8. 本章から学べること

- 外部機器（IoTデバイス等）との連携は、相手側のAPI仕様が事前の想定と異なることが多い。「送信しました」で終わらせず、HTTPステータスやエラー内容を画面に出し、ユーザー自身が原因を切り分けられるようにしておく
- SwiftDataの`@Query`を活用すると、詳細画面の中でデータを編集・削除しても一覧が自動的に追従する。手動で配列を管理し直す必要がなくなり、削除後の状態管理のバグを減らせる
- ゲーム性のための演出（ぼかし・フェード等）は、実用性を損なわない範囲に収まっているか、リリース後の実際のフィードバックで必ず検証し、数値を追い込み直す
- Web版・Kindle原稿・SNSなど、複数のチャネルすべてを「本体験（iOSアプリ）への入り口」として一貫して設計すると、個別のチャネルが小さくても全体としての導線が強くなる
- 「動いているから十分」で終わらせず、無関係な状態変化が重い処理を誤って引き起こしていないか（Reactの依存配列、SwiftUIの再描画）を定期的に疑う。ユーザーからの「遅い」という一言は、たいてい具体的な1箇所の実装に原因があり、計測すれば必ず見つかる
...
- 機能を絞り込む判断（ティール型の「独占ニッチ」戦略など）は強力だが、絶対ではない。実際の利用感で「絞りすぎた」と感じたら、素直に体験を戻す柔軟さも持っておく
- 新しいAI機能を追加する前に、既に生成済みのデータを再利用できないか（キャッシュ・他画面の生成物）を先に確認する。利用料・待ち時間の節約になるだけでなく、画面をまたいだ内容の一貫性という体験上の利点にもつながる
- 「公開」のような1つの概念を、Firestoreのルール・Storageのルールのように複数の場所で別々に判定していると、片方だけ直して片方を直し忘れる不具合が起きやすい。関連する設定を変更する時は、同じ概念を判定している他の場所がないか一緒に洗い出す
- 同じ情報（名前・日付・件数など）を複数の画面に出す時は、項目の並び順まで揃えておく。並び順がバラバラだと、ユーザーは画面ごとに情報を探し直す必要が生まれ、地味に体験を損なう
- 「個別のコンテンツをURLで直接開けるか」は機能一覧には現れにくいが、シェアされて初めて新規訪問者が増えるWeb公開ページでは効果が大きい。ルーティングライブラリを丸ごと導入しなくても、`URLSearchParams`と`history.pushState`だけの小さなフック1つで十分なことも多い
- ヘッダーのボタンのような「常に同じ場所にある操作」も、画面の状態（一覧か詳細か、サインイン済みか等）によって今ユーザーに一番押してほしい導線に出し分けると、案内文が隠れているのに導線も無い、という取りこぼしを防げる
- 「プレビューだけ公開して本編は後日」という状態を長く続けるほど、購入導線を後回しにしがちになる。実際に販売を始めたら、ユーザーがプレビューに辿り着くあらゆる場所（アプリ内・Web版の複数箇所）の購入リンクを一斉に見直す
- 外部サービス（URL短縮など）に依存する機能を追加する時は、そのサービスが使えなかった場合のフォールバック（今回は元のURLをそのまま使う）を必ずセットで用意し、1つの依存先の不調で機能全体が止まらないようにする。ただし、依存先が不安定なこと自体が繰り返し問題になるなら、フォールバックで凌ぎ続けるより「そもそも外部サービスに頼らない設計（今回はUUIDのBase64URLエンコードによる自前の可逆短縮）」に切り替えることも検討する
- 「今まさに画面に見えているもの」をそのまま共有画像・スクリーンショットにできるなら、独自の簡易描画で近似するより、表示中のビューをスナップショットする方が見た目の一貫性が高く実装も単純になりやすい（`GMSMapView`のようなGPU描画ビューは`layer.render(in:)`では撮れず`drawHierarchy(in:afterScreenUpdates:)`が必要、という具体的な落とし穴込みで）
- 可逆変換（エンコード/デコード）を実装する時は、変換ロジック自体の正しさだけでなく、変換した値を実際に照合する相手（今回はFirestoreのドキュメントID）のフォーマット（大文字/小文字など）まで一致しているかを確認する。ロジックは正しくても、フォーマットの揺れだけで一致しない不具合が起こり得る
- 一度直したバグでも、別の操作経路（今回はズーム操作 vs. 古地図の選び直し）から同じ症状が再現することがある。修正後も「この直し方でカバーできていない再現条件はないか」を疑う
- SwiftUIの「型検査タイムアウト」は、直近で追加した複雑な式（インラインのクロージャや`Binding`生成など）を疑い、独立した計算プロパティやサブViewへ切り出すと直ることが多い。1つの`body`に機能を継ぎ足し続けると、いつか型検査の限界を超えるという前提を持っておく

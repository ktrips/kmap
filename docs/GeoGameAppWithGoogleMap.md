...
- 「時空旅を開いた時の画面」と「旅行記画面」で、名前・公開状況・日付・距離・件数といった同じ種類の情報の並び順がバラバラだったため、両方のヘッダーを完全に同じ構成・同じ並び順に揃えました。同じ情報は同じ場所に同じ順で出す、という一貫性は、地味ですがユーザーが画面を覚える負担を減らします
...
いずれも機能としては地味な調整ですが、「同じ情報は同じ場所・同じ順で見せる」「重複した実装を1つにまとめる」「関連する設定は揃っているか確認する」という、当たり前だけれど気を抜くと崩れる原則の積み重ねです。個人開発では大きな新機能よりも、こうした小さな揃え直しの数のほうが、実際の使い心地に効いてきます。

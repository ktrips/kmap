...
### 22-6. 旅行記機能：既存のAI生成資産を再利用する設計

歩いた記録を「その場で振り返る」体験をさらに一歩進め、時間旅の詳細画面に「旅行記を作成する」ボタンを追加しました。旅の名前・使った古地図・巡った御朱印スポット・投稿写真・ユーザー自身の感想を材料に、AIがエッセイ調の旅行記をMarkdown形式で書き上げ、生成後は同じ画面から「旅行記を読む」ボタンでいつでも読み返せます。

**新しいAI呼び出しを増やす前に、既にある生成物を探す**

このアプリには、史跡チェックポイントを開くと「その場所の昔の物語」をAIが生成し、`CheckpointStory`として端末に保存しておく仕組みが既にありました（同じ場所を何度開いても再度AIに問い合わせない、という利用料対策の設計です）。旅行記生成でも、巡った御朱印スポットそれぞれについて安易に新しいAI呼び出しを追加するのではなく、まず「その場所の詳細は既に生成済みか」を`CheckpointStory`から確認し、あればそれをほぼそのまま旅行記に取り込み、無い場合だけその場で生成してキャッシュに保存する設計にしました。
...
この設計には2つの利点があります。1つは単純にAPI利用料と待ち時間の節約です。もう1つは、**アプリの別の画面（チェックポイント詳細シート）と旅行記とで、同じ場所について書かれる内容が一致する**という体験上の一貫性です。プロンプト側にも「取得済みの文章は要約・言い換えをしない」という指示を明示的に与え、AIが同じ場所について毎回違う言い回しを作り出してしまわないようにしています。既存の機能を作るときに用意した「一度生成したら再利用する」という設計判断は、後から生えてくる新機能にとっても資産になる、という一例です。

**AI呼び出しはどこで行うか——クライアント側かサーバー側か**

旅行記生成のAI呼び出しは、既存の物語生成（`AIHistoryService`）と同じく、iOSクライアントから直接OpenAI APIを叩く方式を踏襲しました。Cloud Functions経由でサーバー側からAI APIキーを使う設計（キーを端末に持たせず、利用量の管理もしやすい）も選択肢としてはありましたが、この機能のためだけに新しいCloud Functionsのデプロイ・シークレット管理を増やすコストと、既存の設定画面・Keychain保存の仕組みをそのまま使える手軽さを比べ、後者を選びました。機能ごとに「一貫性を取るか、その機能に最適な設計を取るか」を都度判断する必要があり、今回は既存パターンの一貫性を優先しています。

**文章だけでは物足りない——スクラップブック形式への作り直し**

最初のバージョンは、AIが書いたエッセイをそのまま1本のMarkdownとして表示するだけでした。実際に読んでみると、文章の中に「桜田門を過ぎ、和田倉門へ向かった」と書かれていても、対応する御朱印の写真や歩いたルートの地図がすぐ横に見えないと、記録としての実感が薄いことに気づきました。そこで旅行記の画面を、AIの文章に加えて、実際に歩いたルートの地図・巡った御朱印（写真＋その場所の説明）・投稿写真（写真＋説明）を並べた、スクラップブックのような構成に作り直しました。
...
ここでのポイントは、この「写真＋説明」の一覧を新しいデータ構造として別途持たせるのではなく、御朱印なら`CheckpointStory`、投稿写真なら`WalkPhotoPost.storyBody`という、**既にアプリの中に存在する情報をそのまま並べ直しただけ**という点です。旅行記という新しい画面のために新しいデータを追加で作り込む必要はなく、既存のデータを「1つの画面にまとめて見せる」ビューを1つ書けば済みました。

**生成した内容を、そのままWeb版にも流す**

旅行記の本文（Markdown）は`WalkRoute`に保存され、既存の同期の仕組み（`users/{uid}/walkRoutes`・公開時は`sharedTrips`）にフィールドを2〜3個足すだけで、Web公開ページ（`komap.ktrips.net`）側にもそのまま伝わるようにしました。Web側では新しくAIを呼び出したり、内容を作り直したりはせず、iOSで生成済みのMarkdown本文を、Kindle原稿プレビューで使っているのと同じ`marked`によるMarkdown→HTML変換にそのまま渡すだけです。
...
旅行記機能は、最初の実装で終わりではありませんでした。実際に使ってみて出てきた具体的な要望を、その都度小さな修正として積み重ねています。
...
- 「時空旅を開いた時の画面」と「旅行記画面」で、名前・公開状況・日付・距離・件数といった同じ種類の情報の並び順がバラバラだったため、両方のヘッダーを完全に同じ構成・同じ並び順に揃えました（その後、旅行記画面だけ名前横の古地図名を外し、代わりにサマリー見出し「（古地図名）の時空旅」へ意図的に移すという細部の作り分けも行っていますが、これは揃えた後だからこそできた「意図した差分」です）
- Web版は当初、地図・御朱印ギャラリー・投稿写真ギャラリーが「一般的な一覧セクション」と「旅行記セクション」の2箇所に重複して存在していました（旅行記の有無に関わらず表示される簡易版と、旅行記がある時だけ追加される詳細版）。地図を2つ読み込む無駄も含め、これを1本の流れに統合し、旅行記の有無に関わらず同じ構成で表示するようにしました

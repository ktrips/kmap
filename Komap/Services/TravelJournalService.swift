...
/// 1回の時間旅（`WalkRoute`）の内容から、AIに「旅行記」を書いてもらうサービス。
///
/// 巡った御朱印スポットの詳細は、チェックポイント詳細シート（`CheckpointInfoSheet`）と
/// 同じ`CheckpointStory`キャッシュを使い、既にあればそれをそのまま旅行記に取り込み、
/// 無ければこのタイミングで生成してキャッシュに保存する。これにより、アプリ上の
/// チェックポイント詳細・旅行記・Web上の旅行記表示のすべてで同じ内容になる。
struct TravelJournalService {
    ...
    func generateJournal(
        ...
        「時間旅」の記録（旅の名前・使った古地図・歩いた統計・感想・巡った史跡の詳細・
        投稿した写真の情報）をもとに、旅情あふれる日本語の「旅行記」をエッセイ調で
        書いてください。Markdown形式（`##`程度の小見出しを使ってよい）で、旅の始まりから
        巡った順に史跡を紹介し、最後に短い結びを添えてください。
        各史跡について「その場所の詳細」が与えられている場合は、要約や言い換えをせず、
        与えられた文章をほぼそのまま旅行記の該当箇所に取り込んでください
        （アプリの他の画面でも同じ文章が表示されるため、内容を一致させる必要があります）。
        史実として確定していない場合は、伝承や一般的な当時の様子として、
        断定を避けた表現（例:「〜と伝えられています」「〜だったと考えられます」）を使ってください。
        出力は必ず次の形式のJSONのみとし、それ以外の文字は含めないでください。
        {"title": "20文字程度の旅行記タイトル", "body": "Markdown形式の本文"}
        """

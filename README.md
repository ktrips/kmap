...
- **Apple Watchの電池を守る仕組み**: 使っていない時にWatchアプリが電池を消費しないよう、
  - 一時停止中はGPSを止める（手動の一時停止）か、精度と更新頻度を落とす（動きがない時の自動一時停止。
    動き出したかどうかだけを見る）
  - 最長8時間・動きがない時の自動一時停止の判定は、GPSの更新が来ない静止中も30秒ごとのタイマーで行う
  - 一時停止のまま1時間たったら、止め忘れとみなしてそれまでの軌跡を保存して終える
  - iPhoneは記録中、30秒ごとにWatchへ状態を送り直す（`stateUpdatedAt`）。90秒以上途絶えたら、
    iPhoneのアプリが落ちた・強制終了されたとみなしてWatchの伴走のGPSを止める。古い「記録中」の
    状態が残っていても、Watchを開いただけで伴走が始まることはない。iPhoneのアプリが終了させられた
    時は、その場で「記録していない」状態をWatchへ送り、すぐに伴走を止める
